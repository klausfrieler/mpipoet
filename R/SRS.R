  ...
  psychTestR::new_timeline(
    SRS_NAFC_page(label = sprintf("q%s", item$ID),
                  prompt = shiny::div(
                    ...
                    shiny::p(psychTestR::i18n("PROMPT", sub = list(time_out = timeout)))),
...
SRS_practice <- function(dict = mpipoet::mpipoet_dict){
  ...
  psychTestR::join(
    psychTestR::new_timeline(
      SRS_NAFC_page(label = "ex1",
                    prompt = shiny::div(
                      ...
                      shiny::p(psychTestR::i18n("PROMPT"))),
                    choices = as.character(1:5),
    ...
    # ,
    # psychTestR::new_timeline(
    #   SRS_NAFC_page(label = "ex2",
    #                 prompt = shiny::div(
    #                   shiny::h4(psychTestR::i18n("EXAMPLE")),
    #                   shiny::p(psychTestR::i18n("EXAMPLE_FEEDBACK"))),
    #                 choices = as.character(1:5),
    #                 labels = c(practice_items %>% slice(2) %>% select(starts_with("item")) %>% as.vector(),
...
SRS <- function(num_items = 32L,
  ...
  psychTestR::join(
    ...
    if (with_training) SRS_practice(dict = dict),
